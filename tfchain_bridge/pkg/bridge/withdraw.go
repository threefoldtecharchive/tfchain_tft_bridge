...
func (bridge *Bridge) handlePendingWithdraw(ctx context.Context, pendingWithdraw substrate.WithdrawTransaction) error {
	if err := bridge.wallet.CheckAccount(pendingWithdraw.Target); err != nil {
		...
		return bridge.subClient.RetrySetWithdrawExecuted(ctx, uint64(pendingWithdraw.ID))
	}

	signature, sequenceNumber, err := bridge.wallet.CreatePaymentAndReturnSignature(ctx, pendingWithdraw.Target, uint64(pendingWithdraw.Amount), uint64(pendingWithdraw.ID))
	if err != nil {
		return err
	}
	log.Debug().Msgf("stellar account sequence number: %d", sequenceNumber)

	return bridge.subClient.RetryProposeWithdrawOrAddSig(ctx, uint64(pendingWithdraw.ID), pendingWithdraw.Target, big.NewInt(int64(pendingWithdraw.Amount)), signature, bridge.wallet.GetKeypair().Address(), sequenceNumber)
}
